...
def load_review(path: Path) -> dict[str, Any]:
    try:
        ...
        raise RuntimeError(f"Failed to load threat model review {path.name}: {exc.strerror}") from exc
    except UnicodeError as exc:
        raise RuntimeError(f"Failed to decode threat model review {path.name}: {exc.__class__.__name__}") from exc
    except Exception as exc:
        raise RuntimeError(f"Failed to load threat model review {path.name}: {exc}") from exc
    if not isinstance(data, dict):
        raise RuntimeError("threat model review must be a JSON object")
    checks = data.get("checks")
    if not isinstance(checks, list):
        raise RuntimeError("threat model review requires checks list")
    if not checks:
        raise RuntimeError("threat model review requires at least one check")
    seen: set[str] = set()
...
def run(review_path: Path, reports: Path) -> int:
    ...
    (out_dir / "threat-model-results.json").write_text(json.dumps(result, indent=2, sort_keys=True) + "\n")
...
def main() -> None:
    if len(sys.argv) != 3:
        raise SystemExit("usage: threat_model_review.py REVIEW_JSON REPORTS_DIR")
    try:
